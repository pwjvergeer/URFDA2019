...
This lesson will provide:
 ...
 - Practical advice for writing functions
 - Some suggestions for how to style your code. 
 - The most important thing about code style is to be consistent.
...
You should consider writing a function whenever you've copied and pasted a block of code more than twice (i.e. you now have three copies of the same code). For example, take a look at this code. We have seen this example before. Do you remember what it does?
...
## Think about the input of the function
...
## Extract the input out in general variables
To make the inputs more clear, it's a good idea to rewrite the code using temporary variables with general names. Here this code only requires a single numeric vector, so we'll call it `x`:
...
## Remove duplication by further abstraction (into general variables)
...
## Key steps for creating a new function:
There are three key steps to creating a new function:

1.  Pick a __name__ for the function. Here I've used `rescale01` 
...
3.  You place the code you have developed in __body__ of the function, a 
    `{` block that immediately follows `function(...)`. It is closed by a paring `}`     at the end
...
## Putting the new function to work
...
There is still quite a bit of duplication since we're doing the same thing to multiple columns. We'll learn how to eliminate that duplication in [iteration], once you've learned more about R's data structures in [vectors].
...
## Create fixes in the function-body
...
## see for a solution: https://stackoverflow.com/questions/36753352/distinguishing-between-infinity-and-negative-infinity-during-value-replacement-i


rescale01 <- function(x) {
  
  ## mapping Inf = 1 and -Inf = 0) 
...
B) Practice turning the following code snippets into functions. Think about what each function does. What would you call it? How many arguments does it need? Can you rewrite it to be more expressive or less duplicative? Test your new functions with a dummy.
...
```{r, echo=FALSE}
##becomes
    calculate_amount_NA <- function(x){
      sum(is.na(x))
    }
## test
dummy_1 <- sample(10)
dummy_2 <- c(sample(10), NA, NA)
calculate_amount_NA(x = dummy_1)
calculate_amount_NA(x = dummy_2)
```    
...
```{r, echo=FALSE}
normalize_for_sum <- function(x){
 normalized_x <- x / sum(x, na.rm = TRUE)
 return(normalized_x)
 }

normalize_for_sum(x = dummy_1)
normalize_for_sum(x = dummy_2)
...
```{r, echo=FALSE}
divide_sd_by_mean <- function(x){
  
  sd(x, na.rm = TRUE) / mean(x, na.rm = TRUE)
  
}
divide_sd_by_mean(x = dummy_1)
divide_sd_by_mean(x = dummy_1)

```

C) Follow <http://nicercode.github.io/intro/writing-functions.html> to write your own functions to compute the variance and skew of a numeric vector.

D) Write `both_na()`, a function that takes two vectors of the same length 
    and returns the number of positions that have an `NA` in both vectors.

E) What do the following functions do? Why are they useful even though they are so short?
    
```{r}
is_directory <- function(x) file.info(x)$isdir
is_readable <- function(x) file.access(x, 4) == 0

## You can parse file paths and file names into these  functions which prevents you from typing the complete paths

## They are valuable functions to use in unit-tests

```

## Functions are for humans and computers

 - Functions are not just for the computer
 - They are also for humans. 
 - R doesn't care what your function is called or what the comment means that is in the function
 - These are important for human readers. 

## Naming functions

 - The name of a function is important. 
 - The name of your function will be short
 - Clearly evoke what the function does. 
 - But it's better to be clear than short
 - RStudio's autocomplete makes it easy to type long names.
 - Function names should be verbs
 - Arguments should be nouns. 

## Some naming thoughts

_There are some exceptions: nouns are ok if the function computes a very well known noun (i.e. `mean()` is better than `compute_mean()`), or accessing some property of an object (i.e. `coef()` is better than `get_coefficients()`). A good sign that a noun might be a better choice is if you're using a very broad verb like "get", "compute", "calculate", or "determine". Use your best judgement and don't be afraid to rename a function if you figure out a better name later._
