...
myvars <-c("year","country", "annual_pertussis_cases")
newdata <- filtertest[myvars]
...
newdata %>%
  ...
  toolboxr::rotate_axis_labels(axis="x",
                               angle=90,
                               hjust=1)
```
...
```{r}
log10_pop <- join %>% 
  dplyr::mutate(hallog10 = 
                  (log10(population))) %>%
  select(hallog10, 
         annual_pertussis_cases, 
         country, year) %>%
  arrange(desc(hallog10))
log10_pop
log10_pop %>%
  ggplot(aes(x = year,
             y = hallog10)) +
...
summary_plot
ggsave(filename = "./images/test_dplr.png", plot = summary_plot)
```
Reporting bias?
