...
B) Try the approach you wrote under A on the files in the zip file: "data/diet_data.zip". Unzip the zip-archive using R (`unzip()`). Create a new directory called "diet" where you unzip the files into. Generate a list of csv files paths using `dir()` as explained under A), containing the just unzip[ped csv files from the diet_data.zip archive. Set the `full.names` option to `TRUE`. If all goes well you will end up with five csv files. If you manage to get everything into one dataframe you will have 150 observations and 4 variables in total.   

```{r}
unzip(zipfile = here("data/diet_data.zip"), exdir = here("data/diet"))
files <- dir(path = here("data/diet"), pattern = "\\.csv$", full.names = TRUE)

all_diets <- list()

for(i in seq_along(files)){
  
  all_diets[[i]] <- read_csv(files[[i]])
  
}

all_diets <- dplyr::bind_rows(all_diets)
all_diets

```

C) What happens if you use `for (nm in names(x))` and `x` has no names?
    What if only some of the elements are named? What if the names are
    not unique?

**TIPS** 

 - Generate test vectors with elements that: 1) have no names, 2) are partly named, 3) all have names, 4) Have a duplicate name.

```{r}
x_no_names <- c(letters[1:5])
x_partial_names <- c(aah = "a", bee = "b", "c", "d", eee = "e")
x_names <- c(aah = "a", bee = "b", ceeh = "c", deeeh = "d", eee = "e")
x_names_not_unique <- c(aah = "a", bee = "b", aah = "c", deeeh = "d", eee = "e")
...
## test - no names - the loop completes bu has no output (the input is NULL)
chars <- vector()

for(nm in names(x_no_names)){
  chars[nm] <- nchar(nm)  
}
...
## partial names -  it completes the loop (no names are substituted by and emtpy string "")
chars <- vector()

for(nm in names(x_partial_names)){
  chars[nm] <- nchar(nm)  
}

chars

## full names - it completes the loop
chars <- vector()

for(nm in names(x_names)){
  chars[nm] <- nchar(nm)  
}

chars

## names not unique - it skips the duplicate name
chars <- vector()

for(nm in names(x_names_not_unique)){
  chars[nm] <- nchar(nm)  
}
...
A) Write a function that prints the mean of each numeric column in a data frame, along with its name. For example, `show_mean(iris)` would print:
...
show_mean <- function(df){
  ...
  x <- length(df) ## determine how many columns

  column_means <- vector("numeric", x)

    for(i in seq_along(iris)){
      column_means[[i]] <- mean(iris[[i]], na.rm = TRUE) 
      }
 
   names(column_means) <- column_names
   return(column_means)
  }  
  
## test function
show_mean(df = iris) %>% broom::tidy()
...
1. Solve the problem for a single element of the list. Once
...
A) Adapt `col_summary()` so that it only applies to numeric columns. You might want to start with an `is_numeric()` function that returns a logical vector that has a TRUE corresponding to each numeric column.

```{r}
col_summary <- function(df, fun) {
 
  index_num <- vector("logical", length(df))
  
  for(i in seq_along(df)){
    index_num[[i]] <- is.numeric(df[[i]])
  }
  
  out <- vector("double", length(df[, index_num]))
  for (j in seq_along(df[, index_num])) {
...
col_summary(df = iris, fun = median)
col_summary(df = iris, fun = mean)
col_summary(df = mtcars[, c(5:7)], fun = sum)
```
...
There are a few shortcuts that you can use with `.f` in order to save a little typing. Imagine you want to fit a linear model to each group in a dataset. The following toy example splits up the `mtcars` dataset in to three pieces (one for each value of cylinder) and fits the same linear model to each piece:  
...
```{r, echo=TRUE}
## every second element from the result.
...
A) Write code that uses one (or more) of the map functions to:
    ...
    ```{r}
    ## map2 takes two vectors/recursive vectors as an argument  
    ## take a look at the rnorm()
    mu <- c(-10, 0, 10, 100)
    map(mu, rnorm, n = 10)
...
```{r}
is_col_a_factor <- map_lgl(mtcars, is.factor)
```

## Dealing with failure

When you use the map functions to repeat many operations, the chances are much higher that one of those operations will fail. When this happens, you'll get an error message, and no output. This is annoying: why does one failure prevent you from accessing all the other successes? 

Here you'll learn how to deal with this situation with a new function: `safely()`. `safely()` is an adverb: it takes a function (a verb) and returns a modified version. In this case, the modified function will never throw an error. Instead, it always returns a list with two elements:
...
Let's illustrate this with a simple example: `log10()`:
...
```
 ...
 the arguments are all the same length, it makes sense to store them in a data frame:
...
A) Look at the `mtcars` plots `pwalk` example that saves a number of plots to a pdf. Do the same for the diets dataset, creating single plots for each volunteer. Plot the time on the `Day` on the x-axis and the `Weight` on the y-axis. Provide a discriptive name for the file and save them to your "/sandbox folder.
