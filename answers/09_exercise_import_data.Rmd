...
```
 ...
 4B) Many times data files are compressed. To uncompress a file it depends on the type of archive which function to use. Here we will try to extract the file from ".bz2" archive. This file is in "/data/storm_data_csv.bz2". You will need the package `{R.utils}` and the function from that package `bunzip2()`. Set the `remove` option to FALSE to keep the original archive. 
 
 Extract the file and load the data into R. 
...
```{r}
library(R.utils) 
bunzip2(here("data", "storm_data.csv.bz2"), remove = FALSE, overwrite = TRUE) 
storm <- read_delim(here("data", "storm_data.csv"), delim = ",")
```

 4C) Inspect the storm data. What types of variables are included. Think of a way to get started with a first interesting observation in a visualization. 
...
```
 ...
 4D) Explore the dataset with three different visualizations. You can choose what to plot, but you will have to use `{ggplot}` to do it. Create three appealing graphs with appropriate axis labels and informative titles.
...
```
 ...
 4E) Which county has the most storms? 
...
file.remove(here("data", "storm_data.csv"))
```

4F) Which county has the most injuries


4G) Which county has the most casualties?
